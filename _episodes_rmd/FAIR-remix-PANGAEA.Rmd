...
Let's plan backwards from the the end goal (comparing the temperature proxy measurements):
- Comparing works well visually, so we want to plot a diagram.
- For that, we need to combine/align x- and y-axes from both datasets.
- For that, we need to find out, whether we need to convert them.
- For that, we need to extract the data from the dataset.
- For that, we need to know the datasets' structures.
- For that, we need to download the datasets.
- And we want to do all that in a reproducible manner!
...
> > ](https://ropensci.github.io/pangaear/reference/index.html) gives
> > an overview of its functions. `pg_data()` sounds like what we need.
> {: .solution}
{: .challenge}
